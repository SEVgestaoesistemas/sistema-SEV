...
(() => {
  ...
  window.SevApi = Object.freeze({
    ...
    deleteCrmStage: id => request(`/crm/etapas/${encodeURIComponent(id)}`, { method: 'DELETE', csrf: true }),
    getCrmKanban: async () => (await request('/crm/negociacoes?agrupado_por_etapa=true')).stages,
    getCrmNegotiation: id => request(`/crm/negociacoes/${encodeURIComponent(id)}`),
    createCrmNegotiation: negotiation => request('/crm/negociacoes', { method: 'POST', body: negotiation, csrf: true }),
    updateCrmNegotiation: (id, negotiation) => request(`/crm/negociacoes/${encodeURIComponent(id)}`, { method: 'PATCH', body: negotiation, csrf: true }),
    moveCrmNegotiation: (id, etapaId) => request(`/crm/negociacoes/${encodeURIComponent(id)}/etapa`, { method: 'PATCH', body: { etapaId }, csrf: true }),
    createCrmTask: (id, task) => request(`/crm/negociacoes/${encodeURIComponent(id)}/tarefas`, { method: 'POST', body: task, csrf: true }),
    updateCrmTask: (id, task) => request(`/crm/tarefas/${encodeURIComponent(id)}`, { method: 'PATCH', body: task, csrf: true }),
    deleteCrmTask: id => request(`/crm/tarefas/${encodeURIComponent(id)}`, { method: 'DELETE', csrf: true }),
    createCrmProposalItem: (id, item) => request(`/crm/negociacoes/${encodeURIComponent(id)}/propostas/itens`, { method: 'POST', body: item, csrf: true }),
    deleteCrmProposalItem: id => request(`/crm/propostas/itens/${encodeURIComponent(id)}`, { method: 'DELETE', csrf: true }),
    createCrmHistory: (id, history) => request(`/crm/negociacoes/${encodeURIComponent(id)}/historico`, { method: 'POST', body: history, csrf: true }),
